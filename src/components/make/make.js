...
function Make(props){
    ...
    const [filterList, setFilterList] = useState(makeListCo);
    ...
    const filterBrands = (searchTearm) => {
        ...
        let filteredList = makeList.filter(make => make.name.match(pattern));
    ...
    return(
        <Paper elevation={0} className="makeroot">
            ...
            <div className="makelist">
                {
                         filterList.map((make) => <Brand key={make.id} src={make.logoUrl} data={make} update={updateBrands}/>)
                }
            </div>
